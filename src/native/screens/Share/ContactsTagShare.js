...
import AutoTagsView from './AutoTagsViewShare';
...
class MyAutoTag extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    let uniq = {},
      recentObj = null;
    // console.log(
    //   store.getState().boardList?.boards,
    ...
    recentObj = store
      .getState()
      .messageThreadList?.thread?.threads.map((v) => ({
        ...v?.lastMessage?.from,
      }));
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.queryContactList !== this.props.queryContactList &&
      this.props.queryContactList.length > 0
    ) {
      this.checkAndUpdateSuggestions();
    }
  }
  ...
  render() {
    ...
    return (
      <>
        <View style={{height: 8, backgroundColor: '#FFFFFF'}}></View>
          ...
          <FadeUp
            ...
            <View style={{flexDirection: 'column'}}>
              ...
              <AutoTagsView
                ...
                placeholder="Type and select people"
